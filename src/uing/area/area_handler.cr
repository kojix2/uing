module UIng
  # AreaHandler provides callbacks for Area events with closure support.
  #
  ...
  # 1. AreaHandler MUST remain alive while Area exists
  ...
  class AreaHandler
        ...
        draw: ->(ah : LibUI::AreaHandler*, area : LibUI::Area*, params : LibUI::AreaDrawParams*) {
          begin
            ...
            if !extended.value.draw_box.null?
              callback = ::Box(Proc(Area, AreaDrawParams, Void)).unbox(extended.value.draw_box)
    ...
    def draw(&block : (Area, AreaDrawParams) -> Void)
      @draw_box = ::Box.box(block)
      @extended_handler.draw_box = @draw_box
    end

    def mouse_event(&block : (Area, AreaMouseEvent) -> Void)
    ...
    def key_event(&block : (Area, AreaKeyEvent) -> Bool)
