class AirHockey3DMallet
  property position : Vec2
  property previous : Vec2
  property velocity : Vec2
  property radius : Float64

  def initialize(@position : Vec2, @radius : Float64 = AirHockey3DConfig::MALLET_R)
    @previous = @position
...
class AirHockey3DPuck
  property position : Vec2
  property velocity : Vec2
  property radius : Float64

  def initialize(@position : Vec2, @radius : Float64 = AirHockey3DConfig::PUCK_R)
    @velocity = Vec2.new(0.0, 0.0)
  end
